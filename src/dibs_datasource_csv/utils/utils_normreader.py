...
def get_typ_norm_and_gain_per_person_sia2024(row: pd.DataFrame) -> Tuple[float, str]:
    return (
        float(row["gain_per_person_sia2024"].to_string(index=False).strip()),
        row["typ_sia2024"].to_string(index=False).strip(),
    )


def get_typ_norm_and_gain_per_person_18599(row: pd.DataFrame) -> Tuple[float, str]:
    return (
        float(row["gain_per_person_18599"].to_string(index=False).strip()),
        row["typ_18599"].to_string(index=False).strip(),
    )
